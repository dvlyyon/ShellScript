...
function readLine() {
	while [[ 1 ]]
	do
		read "?please input the predefined number: " line col num
        echo "line:$line col:$col num:$num"
		[[ "$line" == "" ]] && continue
		[[ "$line" == "end"  ]] && break
		{ checkLineCol $line && checkLineCol $col && checkNum $num; } || { echo "line colume number must be in [1, 9]" && continue; }
		sudoku[((($line-1)*9+$col))]=$num
	done
}
...
function readFromFile() {
	...
	while [[ 1 ]]
	do
		read "?please input file name:" fileName
		if [[ -f $fileName ]]
		then
			break
		fi
...
function printSudoku() {
	...
	do
		...
		for (( col=0; col<9; col++))
		do
            if [[ ${sudoku[(($line*9+$col+1))]} == 0 ]]
            then
                echo -n " "
            else
                echo -n ${sudoku[(($line*9+$col+1))]}
            fi
			if (( (col+1)%3 == 0 ))  
			then 
				echo -ne " \u2502 "
